...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('train_10k_batch.log'),
...
class TrainDataProcessor:
    def __init__(self):
        ...
        logger.info(f"当前进度: 已处理 {self.progress['processed_count']} 个样本")
    
    def signal_handler(self, signum, frame):
        """
        信号处理器，优雅退出
        """
        logger.info(f"接收到信号 {signum}，正在保存进度并退出...")
        self.save_progress()
        sys.exit(0)
    ...
    def save_progress(self):
        ...
        try:
            ...
            logger.info(f"进度已保存: 已处理 {self.progress['processed_count']} 个样本")
        except Exception as e:
    ...
    def call_qwen_api(self, post_data: Dict) -> Optional[float]:
        """
        调用Qwen API进行性别倾向评分
        """
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    # 解析失败，返回NaN
                    logger.warning(f"无法解析分数，返回NaN: {content[:50]}...")
                    return float('nan')
                    
                except (ValueError, TypeError):
                    logger.warning(f"分数格式错误，返回NaN: {content[:50]}...")
                    return float('nan')
            else:
    ...
    def process_samples(self, samples: List[Dict]):
        ...
        with tqdm(total=len(samples), desc="处理样本", initial=0) as pbar:
            for i, sample in enumerate(samples):
                try:
                    ...
                    # 调用API获取分数
                    score = self.call_qwen_api(post_data)
        ...
        success_rate = self.progress['success_count'] / self.progress['processed_count'] * 100
        logger.info(f"批次处理完成！总计处理 {self.progress['processed_count']} 个样本，成功率 {success_rate:.1f}%")
    
    def run(self):
        """
        运行主程序
        """
        logger.info("开始训练数据集处理...")
    ...
    def print_status(self):
        ...
        if os.path.exists(self.results_file):
            try:
                ...
                print(f"\n=== 当前状态 ===")
                print(f"已处理样本数: {self.progress['processed_count']}")
                print(f"成功获得分数: {self.progress['success_count']}")
                print(f"CSV文件记录数: {row_count}")
                print(f"运行时间: {int(hours)}小时{int(minutes)}分钟")
                print(f"平均速度: {self.progress['processed_count']/(elapsed_time/3600):.1f}个/小时")
